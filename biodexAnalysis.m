...
for nfiles =1:length(Files)
    ...
    [deMVC.(subject{1,nfiles}).(trial{1,nfiles}).rmvc,metadata.(subject{1,nfiles}).(trial{1,nfiles}).ind_metadata]= importBiodex([PathName,'\',Files{nfiles,1}]);
...
%% for later
% %  pull out the angle location (prox or dist)
% ag = regexp(files,['(?<=X_)\d*(?=\S)'],'match');
% angle = [ag{:,:}] ;
% clear ag
% %  pull out the attachment location (prox or dist)
% at = regexp(files,['(?<=\d_)\w*(?=_\d)'],'match');
% atLoc = [at{:,:}] ;
% clear at
% % pull out the rep number
% rn = regexp(files,['(?<=_)\d$'],'match');
% repN = [rn{:,:}]' ;